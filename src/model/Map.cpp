...
void Map::setTestEnabled(bool testEnabled) {
    Map::testEnabled = testEnabled;
}
...
void Map::showPos(const Pos &pos) {
    if(testEnabled){
        testPos(pos,Point::Type::TEST_VISIT);
    }
}


void Map::showPath(const Pos &start, const std::list<Direction> &path) {
    if(testEnabled){
        Pos tmp=start;
...
std::vector<Pos> Map::getEmptyPoints() const {
    vector<Pos> points;
    SizeType row=getRowCount(),col=getColCount();
    for(SizeType i=1;i<row-1;i++){
        for(SizeType j=1;j<col-1;j++){
